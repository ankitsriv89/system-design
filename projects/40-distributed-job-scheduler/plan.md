...
## Project Brief
Schedule one-off and recurring jobs across workers with leases, retries, priorities, and dependencies.

Primary users: Data pipelines, background task platforms, and internal automation.

Phase: Advanced Data and Platform Systems (36-46)

Recommended stack: Go, PostgreSQL, Redis, gRPC, Docker, Kubernetes notes
...
- Storage layer: PostgreSQL for durable jobs and schedules, Redis for worker heartbeats and fast queues.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache runnable queues by priority while preserving DB as source of truth.
...
Core data model:
- Job(id, type, payload, status, run_at)
- Attempt(job_id, worker_id, status)
- Schedule(id, cron, next_run_at)
...
## Design Decisions
Storage: PostgreSQL for durable jobs and schedules, Redis for worker heartbeats and fast queues.

Consistency: A job attempt is protected by a lease; workers must be idempotent.

Caching: Cache runnable queues by priority while preserving DB as source of truth.
...
## Build Milestones
1. Basic job queue: acceptance is a demo, test, or metric proving the behavior works.
2. workers and leases: acceptance is a demo, test, or metric proving the behavior works.
3. cron scheduler: acceptance is a demo, test, or metric proving the behavior works.
4. DAG and retry policies: acceptance is a demo, test, or metric proving the behavior works.
...
- Explain lease-based scheduling.
- Discuss exactly-once myth.
