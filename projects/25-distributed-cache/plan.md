...
## Project Brief
Build a multi-node cache with sharding, replication, failover, and client routing.

Primary users: Platform teams needing low-latency shared cache infrastructure.

Phase: Distributed Infrastructure (25-35)

Recommended stack: Go, Redis protocol subset, consistent hashing, gossip, Docker
...
- Storage layer: In-memory nodes with optional AOF; cluster metadata persisted in a coordinator or gossip state.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Local node memory plus client-side routing cache.
...
Core data model:
- CacheNode(id, address, status)
- Shard(id, primary, replicas)
- ReplicationEntry(key, version, value)
...
## Design Decisions
Storage: In-memory nodes with optional AOF; cluster metadata persisted in a coordinator or gossip state.

Consistency: Cache is eventually consistent across replicas; primary handles writes for a shard.

Caching: Local node memory plus client-side routing cache.
...
## Build Milestones
1. Single-node cache: acceptance is a demo, test, or metric proving the behavior works.
2. consistent-hash cluster: acceptance is a demo, test, or metric proving the behavior works.
3. replication and failover: acceptance is a demo, test, or metric proving the behavior works.
4. rebalance benchmark: acceptance is a demo, test, or metric proving the behavior works.
...
- Compare Redis Cluster concepts.
- Explain replication lag.
- Discuss client vs proxy routing.
