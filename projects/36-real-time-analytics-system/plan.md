...
## Project Brief
Ingest events and compute near-real-time metrics, funnels, and dashboards.

Primary users: Product analytics, observability, and business intelligence teams.

Phase: Advanced Data and Platform Systems (36-46)

Recommended stack: Python/FastAPI, Kafka, Flink-style workers, ClickHouse, Redis, Grafana
...
- Storage layer: Kafka for event log, ClickHouse for OLAP, Redis for hot aggregates.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache dashboard queries and hot metrics.
...
Core data model:
- Event(id, user_id, type, ts, properties)
- Metric(name, window, value)
- Dashboard(id, queries)
...
## Design Decisions
Storage: Kafka for event log, ClickHouse for OLAP, Redis for hot aggregates.

Consistency: At-least-once ingestion with dedupe by event ID; aggregates tolerate bounded correction.

Caching: Cache dashboard queries and hot metrics.
...
## Build Milestones
1. Event collector: acceptance is a demo, test, or metric proving the behavior works.
2. stream aggregation: acceptance is a demo, test, or metric proving the behavior works.
3. ClickHouse queries: acceptance is a demo, test, or metric proving the behavior works.
4. dashboard and alerts: acceptance is a demo, test, or metric proving the behavior works.
...
- Discuss OLAP schema design.
- Handle dedupe and late arrivals.
