# 27. Design a Netflix Video Streaming Platform

## Project Brief
Ingest videos, transcode variants, manage catalog metadata, and stream through CDN-friendly URLs.

Primary users: Subscription media streaming platform.

Phase: Distributed Infrastructure (25-35)

Recommended stack: Java/Spring Boot, S3-compatible storage, CDN, Kafka, PostgreSQL, FFmpeg workers
...
- Storage layer: PostgreSQL for catalog, object storage for originals/renditions, Kafka for pipeline orchestration.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: CDN caches segments and manifests; Redis caches catalog metadata.
...
- Rendition(video_id, bitrate, object_key)
- PlaybackSession(id, user_id, video_id, license)
...
## Design Decisions
Storage: PostgreSQL for catalog, object storage for originals/renditions, Kafka for pipeline orchestration.

Consistency: Catalog updates are durable; renditions appear after async processing.

Caching: CDN caches segments and manifests; Redis caches catalog metadata.
...
- popular release surge

## Build Milestones
1. Upload/catalog: acceptance is a demo, test, or metric proving the behavior works.
2. transcode worker: acceptance is a demo, test, or metric proving the behavior works.
3. HLS manifest generation: acceptance is a demo, test, or metric proving the behavior works.
4. playback analytics: acceptance is a demo, test, or metric proving the behavior works.
