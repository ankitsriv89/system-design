# 21. Design a Dropbox File Storage and Sync

## Project Brief
Store files, version metadata, sync changes across devices, and support conflict resolution.

Primary users: Cloud file storage and collaboration products.

Phase: Real-Time and Product Systems (14-24)

Recommended stack: Java/Spring Boot, PostgreSQL, S3-compatible storage, Kafka, Redis, desktop sync mock
...
- Storage layer: PostgreSQL for metadata tree, object storage for chunks/files, Kafka as change log.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache folder listings and sync cursors.
...
Core data model:
- FileNode(id, parent_id, owner_id, name, type)
- FileVersion(file_id, version, object_key, checksum)
- SyncCursor(device_id, last_event_id)
...
## Design Decisions
Storage: PostgreSQL for metadata tree, object storage for chunks/files, Kafka as change log.

Consistency: Metadata operations need transactional semantics; sync delivery can be at-least-once.

Caching: Cache folder listings and sync cursors.
...
## Build Milestones
1. Metadata tree API: acceptance is a demo, test, or metric proving the behavior works.
2. chunked upload: acceptance is a demo, test, or metric proving the behavior works.
3. sync cursor feed: acceptance is a demo, test, or metric proving the behavior works.
4. conflict resolution demo: acceptance is a demo, test, or metric proving the behavior works.
...
- Explain metadata vs blob storage.
- Discuss sync cursors and idempotency.
- Handle conflicts and large files.
