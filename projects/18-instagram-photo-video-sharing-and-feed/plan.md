# 18. Design a Instagram Photo/Video Sharing and Feed

## Project Brief
Upload media, process variants, store social graph, and serve ranked feeds.

Primary users: Consumer social media product with media-heavy workloads.

Phase: Real-Time and Product Systems (14-24)

Recommended stack: Java/Spring Boot, Kafka, PostgreSQL, Redis, S3-compatible storage, CDN
...
- Storage layer: PostgreSQL for metadata, object storage for media, Redis for feed/cache, Kafka for processing.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: CDN for media, Redis for feeds and counters.
...
Core data model:
- Media(id, owner_id, object_key, variants)
- Post(id, user_id, media_id, caption)
- Engagement(post_id, user_id, type)
...
## Design Decisions
Storage: PostgreSQL for metadata, object storage for media, Redis for feed/cache, Kafka for processing.

Consistency: Post metadata durable before async media variants finish; feed is eventually consistent.

Caching: CDN for media, Redis for feeds and counters.
...
- deleted media cached

## Build Milestones
1. Upload and metadata: acceptance is a demo, test, or metric proving the behavior works.
2. thumbnail/variant worker: acceptance is a demo, test, or metric proving the behavior works.
3. feed and engagement: acceptance is a demo, test, or metric proving the behavior works.
4. CDN/cache invalidation plan: acceptance is a demo, test, or metric proving the behavior works.
...
- Discuss media pipeline stages.
- Explain fanout for followers.
- Cover counters, ranking, and cache invalidation.
