# 30. Design a Facebook-Like Social Network News Feed

## Project Brief
Design a rich social feed with friendships, groups, pages, ranking, privacy, and reactions.

Primary users: Large social network feed system.

Phase: Distributed Infrastructure (25-35)

Recommended stack: Java/Spring Boot, Kafka, PostgreSQL, Redis, graph store optional
...
- Storage layer: PostgreSQL for entities, Redis for feeds/counters, Kafka for fanout and ranking, graph store optional.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache feed pages with viewer-specific privacy checks.
...
Core data model:
- Story(id, actor_id, target_type, privacy)
- Edge(user_a, user_b, type)
- FeedStory(user_id, story_id, score)
...
## Design Decisions
Storage: PostgreSQL for entities, Redis for feeds/counters, Kafka for fanout and ranking, graph store optional.

Consistency: Privacy changes must remove unauthorized visibility; feed freshness can be eventual.

Caching: Cache feed pages with viewer-specific privacy checks.
...
- privacy regression
- celebrity/page fanout
- ranking drift
- reaction counter lag

## Build Milestones
1. Social graph: acceptance is a demo, test, or metric proving the behavior works.
2. story publish: acceptance is a demo, test, or metric proving the behavior works.
3. privacy-aware feed: acceptance is a demo, test, or metric proving the behavior works.
4. ranking and reactions: acceptance is a demo, test, or metric proving the behavior works.
...
- Explain privacy filtering order.
- Compare graph models.
- Discuss fanout and ranking at scale.
