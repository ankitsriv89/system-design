# 04. Design a Unique ID Generator

## Project Brief
Generate sortable, globally unique IDs at high throughput without a central database bottleneck.

Primary users: Internal services needing primary keys for orders, posts, messages, and events.

Phase: Core Building Blocks (01-13)

Recommended stack: Go, gRPC, PostgreSQL, Docker, Prometheus
...
- Storage layer: PostgreSQL for worker leases and audit records; in-memory counters for fast allocation.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Keep lease state in memory and renew periodically with defensive expiry checks.
...
- POST /v1/ids/next
- POST /v1/ids/batch
- GET /v1/workers/{worker_id}/health

Core data model:
- WorkerLease(worker_id, region, expires_at, sequence_range)
- IdAllocation(ts_ms, worker_id, sequence, id)
- ClockIncident(worker_id, drift_ms, action)
...
- worker lease split brain
- sequence overflow in a millisecond
- region outage

## Build Milestones
1. Local Snowflake generator: acceptance is a demo, test, or metric proving the behavior works.
2. lease service: acceptance is a demo, test, or metric proving the behavior works.
3. batch allocation API: acceptance is a demo, test, or metric proving the behavior works.
4. fault-injection tests for clocks and leases: acceptance is a demo, test, or metric proving the behavior works.
...
- Break down timestamp, worker, and sequence bits.
- Compare UUID, ULID, database sequence, and Snowflake.
- Discuss ordering guarantees and clock risk.
