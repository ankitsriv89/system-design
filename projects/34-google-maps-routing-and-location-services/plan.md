# 34. Design a Google Maps Routing and Location Services

## Project Brief
Model places, roads, traffic, routing, geocoding, and location-aware queries.

Primary users: Maps, logistics, delivery, and mobility apps.

Phase: Distributed Infrastructure (25-35)

Recommended stack: Java/Go, PostgreSQL/PostGIS, graph engine, Redis, Kafka
...
- Storage layer: PostGIS for places, graph storage for road network, Redis for route and tile caches.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache popular routes, geocoding results, and map tiles.
...
Core APIs:
- GET /v1/geocode
- GET /v1/routes?from=&to=
- POST /v1/traffic-events

Core data model:
- Place(id, name, lat, lng)
- RoadEdge(from, to, distance, speed)
- TrafficEvent(edge_id, speed, expires_at)
...
## Design Decisions
Storage: PostGIS for places, graph storage for road network, Redis for route and tile caches.

Consistency: Traffic is eventually consistent; route requests use a timestamped graph snapshot.

Caching: Cache popular routes, geocoding results, and map tiles.
...
## Build Milestones
1. Place search: acceptance is a demo, test, or metric proving the behavior works.
2. road graph import: acceptance is a demo, test, or metric proving the behavior works.
3. shortest path API: acceptance is a demo, test, or metric proving the behavior works.
4. traffic-aware routing: acceptance is a demo, test, or metric proving the behavior works.
...
- Explain Dijkstra/A* tradeoffs.
- Discuss map tiling and geospatial indexes.
- Handle live traffic updates.
