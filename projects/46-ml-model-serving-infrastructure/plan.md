# 46. Design a Machine Learning Model Serving Infrastructure

## Project Brief
Serve ML predictions with versioning, canaries, feature lookup, batching, and monitoring.

Primary users: ML platform and applied AI production teams.

Phase: Advanced Data and Platform Systems (36-46)

Recommended stack: Python/FastAPI, model registry, Redis, Kafka, Kubernetes, Prometheus
...
- Storage layer: Object storage for artifacts, PostgreSQL for registry, Redis/feature store for online features, Kafka for logs.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache loaded models and hot features; batch compatible requests.
...
Core data model:
- ModelVersion(model, version, artifact_uri, status)
- PredictionRequest(id, features)
- PredictionLog(model, version, latency, output)
...
## Design Decisions
Storage: Object storage for artifacts, PostgreSQL for registry, Redis/feature store for online features, Kafka for logs.

Consistency: Prediction path must pin a model version per request; promotion is controlled and auditable.

Caching: Cache loaded models and hot features; batch compatible requests.
...
## Build Milestones
1. Registry API: acceptance is a demo, test, or metric proving the behavior works.
2. prediction service: acceptance is a demo, test, or metric proving the behavior works.
3. canary routing: acceptance is a demo, test, or metric proving the behavior works.
4. monitoring and rollback: acceptance is a demo, test, or metric proving the behavior works.
...
- Discuss online/offline feature consistency.
- Handle canaries and rollback.
