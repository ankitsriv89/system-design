# 19. Design a Twitter/X Timeline and Posts

## Project Brief
Support short posts, follows, timelines, search, trends, and high-fanout accounts.

Primary users: Public social network and microblogging platform.

Phase: Real-Time and Product Systems (14-24)

Recommended stack: Java/Spring Boot, Kafka, PostgreSQL, Redis, OpenSearch
...
- Storage layer: PostgreSQL for tweets/graph, Redis for timelines, Kafka for fanout/search/trends.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache home timelines, user timelines, and engagement counters.
...
Core data model:
- Tweet(id, author_id, text, created_at)
- Follow(follower_id, followee_id)
- TimelineItem(user_id, tweet_id, score)

Events:
- tweet.created
- tweet.deleted
- timeline.fanout_completed

## Design Decisions
Storage: PostgreSQL for tweets/graph, Redis for timelines, Kafka for fanout/search/trends.

Consistency: Durable tweet creation; timelines and search are eventually consistent.

Caching: Cache home timelines, user timelines, and engagement counters.
...
## Build Milestones
1. Tweet/follow API: acceptance is a demo, test, or metric proving the behavior works.
2. home timeline: acceptance is a demo, test, or metric proving the behavior works.
3. search indexer: acceptance is a demo, test, or metric proving the behavior works.
4. celebrity hybrid fanout: acceptance is a demo, test, or metric proving the behavior works.
...
- Compare Twitter timeline strategies.
- Discuss hot accounts and ranking.
- Explain eventual consistency for search and trends.
