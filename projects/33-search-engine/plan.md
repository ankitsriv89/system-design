# 33. Design a Search Engine

## Project Brief
Crawl, parse, index, rank, and query documents with relevance and freshness tradeoffs.

Primary users: Web, marketplace, docs, or internal search products.

Phase: Distributed Infrastructure (25-35)

Recommended stack: Java/Python, crawler, Kafka, OpenSearch/Lucene, PostgreSQL
...
- Storage layer: OpenSearch/Lucene for index, PostgreSQL for metadata, Kafka for indexing pipeline.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache hot queries and term dictionaries.
...
Core data model:
- Document(id, url, title, body, checksum)
- Posting(term, doc_id, tf)
- SearchClick(query, doc_id, rank)
...
## Design Decisions
Storage: OpenSearch/Lucene for index, PostgreSQL for metadata, Kafka for indexing pipeline.

Consistency: Search index is eventually consistent; canonical documents are durable.

Caching: Cache hot queries and term dictionaries.
...
## Build Milestones
1. Document ingestion: acceptance is a demo, test, or metric proving the behavior works.
2. inverted index search: acceptance is a demo, test, or metric proving the behavior works.
3. ranking signals: acceptance is a demo, test, or metric proving the behavior works.
4. crawler plus reindex pipeline: acceptance is a demo, test, or metric proving the behavior works.
...
- Discuss freshness vs query latency.
- Handle ranking and spam.
