...
## Project Brief
Deliver realtime private messages with online presence, history, receipts, and offline push.

Primary users: Direct messages in social, support, or collaboration apps.
...
- Storage layer: PostgreSQL for durable history, Kafka for fanout, Redis for sessions and presence.
- Worker or stream processor for asynchronous processing and retries.
- Cache/read path: Cache recent conversations and online presence.
...
Core data model:
- Conversation(id, user_a, user_b)
- Message(id, conversation_id, sender_id, body, seq, status)
...
## Design Decisions
Storage: PostgreSQL for durable history, Kafka for fanout, Redis for sessions and presence.

Consistency: Ordering per conversation using sequence assignment.

Caching: Cache recent conversations and online presence.
...
## Build Milestones
1. REST conversation API: acceptance is a demo, test, or metric proving the behavior works.
2. WebSocket send/receive: acceptance is a demo, test, or metric proving the behavior works.
3. delivery/read receipts: acceptance is a demo, test, or metric proving the behavior works.
4. offline push integration: acceptance is a demo, test, or metric proving the behavior works.
...
- Explain message ordering per conversation.
- Discuss WebSocket scaling and sticky sessions.
- Handle idempotent sends.
